...
6. Playlistの食用方法：将该文件直接拖到支持m3u/dpl播放列表的播放器(推荐使用Potplayer)内就好啦，因为其中保存的是相对位置文件位置，请不要更改该课件的文件结构哦，否则该列表会失效的哦
...
6. 新增播放列表playlist.dpl，增大播放列表的兼容性，暂时保留旧播放列表playlist.m3u
